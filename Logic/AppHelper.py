...
def create_auth_token(session, length=64):
    ...
    auth_control = AuthControl(Auth_token=auth_token, role="SET ROLE", expiry=(
...
def delete_old_auth_Tokens(session):
    ...
    session.execute(
        text('DELETE FROM "Auth_Access_Control" WHERE expiry < CURRENT_TIMESTAMP'))
...
def checkRole(request, session):
    ...
    if ('auth_token' in cookies):
        ...
        stmt = select(AuthControl).where(
            AuthControl.Auth_token == auth_token_value)
...
def static_login_cookie(role):
    resp = make_response()
    auth_token = role
    resp.set_cookie('auth_token', auth_token, expires=datetime.utcnow(
